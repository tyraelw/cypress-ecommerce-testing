  ...
  cypress-run:
    ...
    steps:
      ...
      - name: Create environment file
        run: |
          echo '{
            "defaultEmail": "${{ secrets.CYPRESS_EMAIL }}",
            "defaultPassword": "${{ secrets.CYPRESS_PASSWORD }}",
...
- `CYPRESS_EMAIL` - Valid login email
...
pipeline {
    ...
    stages {
        stage('Checkout') {
            steps {
                git branch: 'main',
                    url: 'https://github.com/tyraelw/cypress-ecommerce-testing.git'
            }
        }
        ...
        stage('Create Environment File') {
            steps {
                withCredentials([
                    string(credentialsId: 'cypress-email', variable: 'EMAIL'),
                    string(credentialsId: 'cypress-password', variable: 'PASSWORD')
                ]) {
                    sh '''
                        echo "{
                            \\"defaultEmail\\": \\"$EMAIL\\",
